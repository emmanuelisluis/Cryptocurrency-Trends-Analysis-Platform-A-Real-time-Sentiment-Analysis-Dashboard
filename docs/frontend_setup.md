...
*   **Pages (`frontend/src/pages/`):**
    ...
    *   `NewsSentimentPage.tsx`: Provides a dedicated view for news articles and their associated sentiment.
*   **Chart & Data Display Components (`frontend/src/components/market/` and `frontend/src/components/news/`):** These are the core analytical and informational tools.
    *   **Market Analysis Components (`frontend/src/components/market/`):**
    ...
    *   **News & Sentiment Components (`frontend/src/components/news/`):**
        *   `NewsDisplay.tsx`: A component responsible for fetching, listing, and displaying news articles, including their (simulated) sentiment analysis results (label, score, and visual indicator).
*   **API Service (`frontend/src/services/marketDataService.ts`):**
    ...
    *   Defines comprehensive TypeScript interfaces for all API request parameters and response payloads, ensuring type safety throughout the frontend application. This includes interfaces for order book data, trades, footprint data, volume profiles, CVD, advanced delta metrics, ML prediction inputs/outputs, news articles, and sentiment analysis.
